...
export const LayDanhSachNguoiDungPhanTrang = () => {
  return (dispatch) => {
    Axios({
      url:
        "https://movie0706.cybersoft.edu.vn/api/QuanLyNguoiDung/LayDanhSachNguoiDungPhanTrang?MaNhom=GP01&soTrang=1&soPhanTuTrenTrang=10            ",
      method: "GET",
    })
      .then((result) => {
        console.log(result.data);
  ...
  return (dispatch) => {
    ...
    Axios({
      ...
      .then((result) => {
        ...
        swal.fire("", `Xóa Khoản Thành Công Tài Khoản: ${taiKhoan}`, "success");
      })
      .catch((err) => {
        swal.fire("", `${err.response.data}`, "error");
        console.log(err);
